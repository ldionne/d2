...
template <typename Graph, typename Segment, bool SilentlyIgnoreOtherEvents>
...
/**
 * Algorithm building a segmentation graph from a range of events.
 * The events in the range should be `start`s or `join`s. Depending on the
 * `SilentlyIgnoreOtherEvents` template parameter, other events will be
 * ignored or an `UnexpectedEventException` will be thrown.
 *
 * Either way (`SilentlyIgnoreOtherEvents` or not), the algorithm provides
...
void build_segmentation_graph(Range const& range, Graph& graph) {
    build_segmentation_graph<SilentlyIgnoreOtherEvents>(
